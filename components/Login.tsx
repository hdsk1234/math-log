...
import { registerTeacher } from '../lib/db';
import { UserRole } from '../types';

interface Props {
  onStudentLogin: (pinHash: string) => Promise<boolean>; // Accepts Hash
}

// In a real app, verify this on the backend (Cloud Functions)
...
export const Login: React.FC<Props> = ({ onStudentLogin }) => {
  ...
  const [showPin, setShowPin] = useState(false); // Visibility toggle
  ...
  useEffect(() => {
    // Load attempts and lockout from local storage
  ...
  // Timer to clear lockout automatically
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedRole === 'teacher') {
        // Teacher Login via Firebase Auth
        if (isSignUp) {
        ...
        // [Security] Hash PIN before sending
      ...
      if (err.message === "SECURE_CONTEXT_REQUIRED") {
        setLocalError('보안 환경(HTTPS 또는 localhost)에서만 로그인이 가능합니다. 모바일 테스트 시 HTTPS를 적용해주세요.');
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      console.error(err);
      setLocalError('Google 로그인에 실패했습니다. (팝업이 차단되었거나 승인되지 않은 도메인일 수 있습니다)');
    } finally {
      setIsLoading(false);
    }
  };
