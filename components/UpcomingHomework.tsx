...
import { UpcomingHomeworkData } from '../types';
...
export const UpcomingHomework: React.FC<Props> = ({
  data,
  isAdmin,
  onAddSchedule,
  onDeleteSchedule,
              ...
              <div key={sIdx} className={`flex gap-4 group/schedule relative transition-opacity ${isPast ? 'opacity-60 grayscale-[0.5]' : ''}`}>
                ...
                <div className="flex-shrink-0 w-14 pt-1 flex flex-col items-center gap-1">
                  ...
                  {isAdmin && (
                    <button
                      onClick={() => {
                        if (window.confirm('정말 이 숙제 일정을 삭제하시겠습니까?')) {
                  ...
                  {schedule.categories.map((category, cIdx) => (
                    <div key={cIdx} className="relative">
                      ...
                      {category.items.length > 0 ? (
                        ...
                        !isAdmin && <p className="text-xs text-gray-300 pl-3 italic">숙제 없음</p>
                      )}
                    </div>
                  ))}
                </div>
