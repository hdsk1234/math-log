...
my $filepath = 'pages/StudentJournalDashboard.tsx';
...
my $target1 = '  const togglePaymentPaidStatus = (dateStr: string) => {';
my $replacement1 = <<'EOF';
...
if (index($content, $target1) != -1) {
    $content =~ s/\Q$target1\E/$replacement1/;
    print "Successfully injected SMS states/functions.\n";
} else {
    print "Failed to locate togglePaymentPaidStatus.\n";
    exit 1;
}
...
if ($content =~ s/<div className="w-full bg-white\/10 h-1\.5 rounded-full mt-2 overflow-hidden">[\s\S]*?<div\s*className={`h-full transition-all duration-300 \$\{[\s\S]*?completedLessonsSincePayment >= \(student\.profile\.lessonFeeCycle \|\| 8\)[\s\S]*?수업료 수납 완료 여부는[\s\S]*?<\/div>/$replacement2/) {
    print "Successfully restored broken JSX layout.\n";
} else {
    print "Failed to match broken JSX layout regex.\n";
    exit 1;
}

open(my $out, '>:encoding(UTF-8)', $filepath) or die "Cannot write to $filepath: $!";
print $out $content;
close($out);
